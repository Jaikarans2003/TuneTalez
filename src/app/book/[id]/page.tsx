...
import { getBookById, BookDocument, Chapter, updateBook } from '@/firebase/services';
...
export default function ViewBookPage() {
    ...
    try {
      ...
      if (book.audioUrl?.startsWith('blob:')) {
        ...
        try {
          ...
          // Upload the valid MP3 file
          await uploadBytes(storageRef, validAudio);
          
          // Get the download URL
          const newAudioUrl = await getDownloadURL(storageRef);
  ...
  const playAudio = async (audioUrl: string) => {
    ...
    try {
      ...
      if (!httpsUrl) {
        console.error('❌ Invalid audio URL. Could not get a valid Firebase HTTPS URL:', audioUrl);
        return;
      }
