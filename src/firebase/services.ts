...
// Upload audio narration file to Firebase Storage
export const uploadAudioNarration = async (
  ...
  try {
    console.log('Starting audio upload to Firebase Storage:', { bookId, chapterId, fileSize: file.size, retryAttempt });
      ...
      uploadTask.on(
        ...
        async (error) => {
          ...
          if (retryAttempt < 1) {
            ...
            try {
              ...
              const url = await uploadAudioNarration(file, bookId, chapterId, onProgress, retryAttempt + 1);
              resolve(url);
            } catch (retryError) {
        ...
        async () => {
          ...
          try {
            ...
            // Retry getting the URL if this is the first attempt
            if (retryAttempt < 1) {
              console.log('Retrying to get download URL...');
              try {
    ...
    console.error('Error in uploadAudioNarration:', error);
    
    // Retry once if this is the first attempt
    if (retryAttempt < 1) {
      console.log('Retrying entire upload process...');
      try {
        await new Promise(r => setTimeout(r, 2000));
        return await uploadAudioNarration(file, bookId, chapterId, onProgress, retryAttempt + 1);
      } catch (retryError) {
