  ...
  for (const idx of indexesToTry) {
    try {
      ...
      try {
        ...
        const musicRefUppercase = ref(storage, pathUppercase);
        url = await getDownloadURL(musicRefUppercase);
        filename = filenameUppercase;
        foundIndex = idx;
        found = true;
        console.log(`Found background music file with uppercase extension: ${filenameUppercase}`);
        break;
      } catch (upperCaseErr: any) {
        if (upperCaseErr.code === 'storage/object-not-found') {
          // Try lowercase extension
          try {
            const pathLowercase = `BackgroundMusic/${category}/${filenameLowercase}`;
            console.log(`Trying to access: ${pathLowercase}`);
            const musicRefLowercase = ref(storage, pathLowercase);
            url = await getDownloadURL(musicRefLowercase);
            ...
            if (lowerCaseErr.code === 'storage/object-not-found') {
              console.warn(`Background music file not found with either case: ${category}_${idx}.MP3 or ${category}_${idx}.mp3`);
              continue; // Try next index
            }
            throw lowerCaseErr; // Throw other errors from lowercase attempt
          }
        }
        throw upperCaseErr; // Throw other errors from uppercase attempt
      }
    } catch (err: any) {
...
export const listBackgroundMusicCategories = async (): Promise<string[]> => {
  try {
    const musicRef = ref(storage, 'BackgroundMusic');
...
export const listBackgroundMusicInCategory = async (category: string): Promise<string[]> => {
  try {
    const categoryRef = ref(storage, `BackgroundMusic/${category}`);
...
export const getRandomBackgroundMusic = async (category: MusicCategory): Promise<BackgroundMusic> => {
  try {
    ...
    console.log(`Selected random file: ${randomFile.name} from category: ${category}`);
    
    // Get download URL
    const url = await getDownloadURL(randomFile);
    
    // Extract index from filename (e.g., "Happy_3.mp3" -> 3)
    const filenameMatch = randomFile.name.match(/(\d+)\.mp3$/i);
