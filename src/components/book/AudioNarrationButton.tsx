...
import { ensureFirebaseUrl, ensureFirebaseHttpsUrl } from '@/utils/audioUtils';
...
const AudioNarrationButton = ({ 
  ...
  const handleGenerateNarration = async () => {
    ...
    try {
      ...
      // Step 6: Validate and set the audio URL to the Firebase Storage URL
      ...
      if (storageUrl.startsWith('gs://')) {
        ...
        validUrl = await ensureFirebaseHttpsUrl(storageUrl);
        ...
        validUrl = ensureFirebaseUrl(storageUrl);
        
        if (!validUrl) {
          throw new Error('Invalid Firebase Storage URL received from uploadAudioNarration');
        }
      }
      
      console.log('✅ Setting audio URL to Firebase Storage URL:', validUrl);
      ...
      if (bookId) {
        ...
        console.log('📝 Updating book/chapter with Firebase Storage URL');
        if (chapterId) {
      ...
      // Call success callback with the validated Firebase Storage URL
