...
import { ensureFirebaseUrl, ensureFirebaseHttpsUrl, validateAudioUrl, normalizeAudioUrl } from '@/utils/audioUtils';
...
const AudioPlayer = ({ 
  ...
  useEffect(() => {
    ...
    // Handle potential gs:// URIs and ensure we have a valid Firebase Storage URL
    const processUrl = async () => {
      try {
        ...
        if (audioUrl.startsWith('gs://')) {
          ...
          const httpsUrl = await ensureFirebaseHttpsUrl(audioUrl);
          
          if (!httpsUrl) {
            setError('Failed to convert gs:// URI to HTTPS URL. Please provide a valid Firebase Storage URL.');
            setIsLoading(false);
            return;
          }
        ...
        const validUrl = ensureFirebaseUrl(audioUrl);
        
        if (!validUrl) {
          console.error('❌ Invalid audio URL rejected:', audioUrl);
          console.error('Component stack:', new Error().stack);
          setError('Invalid audio URL. Only Firebase Storage HTTPS URLs are supported.');
          setIsLoading(false);
          return;
        }
        
        // Log the valid URL being used
        console.log('✅ Using Firebase HTTPS URL for audio:', validUrl);
    ...
    const handleError = (e: ErrorEvent) => {
      ...
      if (audioUrl.startsWith('blob:')) {
        detailedError += ' Blob URLs may not work in deployed environments.';
      }
